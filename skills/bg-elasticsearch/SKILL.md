...
- uses `BG_ELASTIC_PYTHON` when it is set
- otherwise uses `BG_ELASTIC_VENV/bin/python` when `BG_ELASTIC_VENV` is set
- uses `BG_VAULT_ELASTIC_DIR` when it is set
- otherwise defaults to `$HOME/bg-vault-client/bg_vault_elastic`
- prepends `$BG_VAULT_ELASTIC_DIR/src` to `PYTHONPATH` when running a configured Python
- otherwise runs the code with `uv run python` from that checkout
